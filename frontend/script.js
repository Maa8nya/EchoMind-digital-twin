...
if (femaleVoice) {
    ...
    utterance.onerror = (e) => {
        console.error(e);
        Terminal.log("TTS_ERROR: " + e.error, 'error');
        this.stopTTS();
    };
...
if (downloadBtn) {
    downloadBtn.addEventListener('click', () => {
        ...
        document.querySelectorAll('#chat-history .message').forEach(msg => {
            ...
            let role = "user";
            if (roleLabel.toLowerCase().includes("Maanya")) role = "model";
        ...
        .then(blob => {
            ...
            a.href = url;
            a.download = `ECHO_MIND_REPORT_${new Date().getTime()}.pdf`;
...
function handleSendMessage() {
    ...
    const currentSeniority = senioritySlider ? parseInt(senioritySlider.value) : 0;
    ...
    .then(data => {
        ...
        addMessage(data.response, 'ai-message', "Maanya's digital twin");
    })
    .catch(error => {
